...
LOG_FILE=$PWD/ffmpeg_transcode.log
...
function transcode() {
	...
	ffmpeg -y -i $INPUT_FILE -s 1920x1080 -b 6000k -r 30 -vcodec h264_videotoolbox -ab 64k -ar 44.1k -ac 1 -acodec aac $OUTPUT_DIR/ffmpeg_mac_1080.mp4
	echo "1080: end time $(date +%s)" >> $LOG_FILE

	echo "720: start time $(date +%s)" >> $LOG_FILE
	ffmpeg -y -i $INPUT_FILE -s 1280x720 -b 4000k -r 30 -vcodec h264_videotoolbox -ab 64k -ar 44.1k -ac 1 -acodec aac $OUTPUT_DIR/ffmpeg_mac_720.mp4
	echo "720: end time $(date +%s)" >> $LOG_FILE

	echo "540: start time $(date +%s)" >> $LOG_FILE
	ffmpeg -y -i $INPUT_FILE -s 960x540 -b 3000k -r 30 -vcodec h264_videotoolbox -ab 64k -ar 44.1k -ac 1 -acodec aac $OUTPUT_DIR/ffmpeg_mac_540.mp4
	echo "540: end time $(date +%s)" >> $LOG_FILE

	echo "480: start time $(date +%s)" >> $LOG_FILE
	ffmpeg -y -i $INPUT_FILE -s 640x480 -b 2000k -r 30 -vcodec h264_videotoolbox -ab 64k -ar 44.1k -ac 1 -acodec aac $OUTPUT_DIR/ffmpeg_mac_480.mp4
